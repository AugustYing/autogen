...
LEDGER_DIGRAPH_TASK_LEDGER_PLAN_PROMPT = """Fantastic. To address this request we have assembled the following team:
...
IMPORTANT: Your output must strictly match the schema below. Do not add any extra fields, comments, or formatting.
...
Please output an answer in pure JSON format according to the following schema. The JSON object must be parsable as-is. DO NOT OUTPUT ANYTHING OTHER THAN JSON, AND DO NOT DEVIATE FROM THIS SCHEMA. DO NOT INCLUDE CODE BLOCK MARKERS (such as ```json or ```).

Example:
{{
   "completed": {{
        "reason": "The request has not been fully addressed because ...",
        "answer": false
    }}
}}
...
LEDGER_DIGRAPH_TASK_LEDGER_FACTS_UPDATE_PROMPT = """As a reminder, we are working to solve the following task:
...
LEDGER_DIGRAPH_TASK_LEDGER_PLAN_UPDATE_PROMPT = """Please briefly explain what went wrong on this last run (the root cause of the failure), and then come up with a new plan that takes steps and/or includes hints to overcome prior challenges and especially avoids repeating the same mistakes. As before, the new plan should be concise, be expressed in bullet-point form, and consider the following team composition (do not involve any other outside people since we cannot contact anyone else):
